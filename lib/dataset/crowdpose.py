...
class CROWDPOSEDataset(JointsDataset):
	...
	"skeleton": [
        [12,13],[13,0],[13,1],[0,2],[2,4],[1,3],[3,5], [13,7],[13,6],
        [7,9],[9,11],[6,8],[8,10]]
    '''
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        self.pixel_std = 200
        name = self._get_ann_file_keypoint()
    ...
    def _get_ann_file_keypoint(self):
        ...
        return os.path.join(
            self.root,
            'json',
            'crowdpose_' + self.image_set + '.json'
        )
        ...
        for img in kpts.keys():
            ...
            if self.soft_nms:
                keep = soft_oks_nms(
                    ...
                    oks_thre, num_joints=num_joints
                )
            else:
                keep = oks_nms(
                    [img_kpts[i] for i in range(len(img_kpts))],
